  ...
  return (
    <div className="inline-block overflow-hidden rounded-xl">
      ...
      <div className="w-full absolute bottom-0 p-10 z-20">
        <Tag link={`categories/${post.tags[0]}`} name={post.tags[0]} className="px-6 text-sm py-2 !border" />
        <Link href={post.url} className="mt-6">
          <h2 className="font-bold capitalize text-2xl text-white mt-4">
            <span className="bg-gradient-to-r from-violet-500 to-violet-500 bg-[length:0px_6px] hover:bg-[length:100%_6px] bg-left-bottom bg-no-repeat transition-[background-size] duration-500">
              {post.title}
            </span>
          </h2>
        </Link>
